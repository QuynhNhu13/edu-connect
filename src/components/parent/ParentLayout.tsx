...
const ParentLayout = () => {
  ...
  const { profile, notifications, markNotificationRead, markAllNotificationsRead, chatMessages } = useParent();
  const [collapsed, setCollapsed] = useState(false);
  const [showNotif, setShowNotif] = useState(false);
  const notifRef = useRef<HTMLDivElement>(null);

  const unreadNotif = notifications.filter(n => !n.read).length;
  const unreadChat = chatMessages.filter(m => !m.read && m.sender !== "parent").length;
        ...
        <div className="h-16 flex items-center gap-3 px-4 border-b border-border">
          ...
          {!collapsed && (
            <div className="min-w-0">
              ...
              <p className="text-[11px] opacity-70 leading-tight">Phụ huynh</p>
            </div>
          )}
        ...
        <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
          ...
          {navItems.map(item => (
            <NavLink key={item.to} to={item.to} end={item.end} title={collapsed ? item.label : undefined}
              ...
              {item.to === "/parent/chat" && unreadChat > 0 && (
                <span className={cn("flex items-center justify-center text-[10px] font-bold rounded-full bg-destructive text-destructive-foreground", collapsed ? "absolute -top-1 -right-1 min-w-[16px] h-4 px-1" : "min-w-[20px] h-5 px-1.5")}>{unreadChat}</span>
              )}
            </NavLink>
          ))}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-border bg-card px-6 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <UserAvatarDropdown avatar={profile.avatar} name={profile.name} role="Phụ huynh" />
          </div>
        </header>
